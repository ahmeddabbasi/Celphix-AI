class SalesAgentApp {
    ...
    async initializeClientVAD() {
        try {
            ...
            if (typeof SileroVADClient === 'undefined') {
                console.warn(' SileroVADClient not available, using fallback energy VAD');
                return;
            }
            ...
            this.clientVAD.setCallbacks({
                onSpeechStart: (speechProb) => {
                    console.log(` VAD: Speech started (prob: ${speechProb.toFixed(3)})`);
                ...
                onVADUpdate: (speechProb, isSpeaking) => {
                    ...
                    if (this.isAiSpeaking && isSpeaking && speechProb > this.vadConfig.interruptThreshold) {
                        ...
                        // STEP 1: INSTANT STOP - Use immediate stop for zero-latency interruption
        ...
        this.serverSpeechStartTime = 0; // Track single words in server VAD
            ...
            console.warn(' Failed to initialize Silero VAD:', error);
    ...
    handleWebSocketMessage(event) {
        ...
        switch (message.type) {
            ...
            case 'no_calling_index':
                console.log(' No calling index found');
            ...
            case 'next_customer_activated':
                console.log(' Next customer activated');
                if (message.customer_index) {
            ...
            this.workletNode.port.onmessage = async (event) => {
                ...
                if (this.vadEnabled && this.clientVAD && type === 'audioData') {
                    try {
                        ...
                        console.warn(' Client VAD processing error:', error);
                    }
                }
            };
    ...
    clearAllAudioBuffersForced() {
        console.log(' FORCED DEEP CLEAN: Emergency clearing of ALL audio state');
