...
class DownsampleTo16kPCM16Processor extends AudioWorkletProcessor {
    constructor() {
        ...
        // AI speaking state (for interrupt detection)
        this._isAiSpeaking = false;
        this._lastInterruptTime = 0;
        
        // Listen for VAD configuration updates and AI state changes
        this.port.onmessage = (event) => {
            ...
            if (type === 'aiSpeakingState') {
                this._isAiSpeaking = data.isAiSpeaking;
            }
        };
    }
    ...
    _processVAD(audioChunk) {
        try {
            ...
            // INTERRUPT DETECTION: If AI is speaking and user starts speaking, send interrupt signal
            if (this._isAiSpeaking && isSpeaking && stateChanged) {
                const now = Date.now();
                // Debounce interrupts (200ms minimum between interrupts)
                if (now - this._lastInterruptTime > 200) {
                    ...
                    console.log('⚡ INTERRUPT detected in worklet');
                }
            }
